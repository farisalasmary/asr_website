...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function download_srt() {
    ...
    for (i = 0; i < spans.length; i += num_words_per_sentence) {
      words = ''; // add 500 milliseconds to the start time of each segment except the first segment
  ...
  function add_text_onclick_event() {
    for (i = 0; i < words.length; i++) {
      document.getElementById("word_" + i).addEventListener("click", setCurTime, false);
    }
  }

  function remove_text_onclick_event() {
    for (i = 0; i < words.length; i++) {
      document.getElementById("word_" + i).removeEventListener("click", setCurTime, false);
    }
  }
...
/******/ });
//# sourceMappingURL=downloader-bbbdb2a586dc129df168.js.map